...
/**
 * Constants for the `sourceType` column used across songs, albums, and artists.
 */
object SourceType {
...
/**
 * Unified song representation for all source types.
 */
@Entity(
    tableName = "songs",
    indices = [
...
interface AlbumDao {
    ...
    @Query("DELETE FROM albums WHERE id NOT IN (SELECT DISTINCT albumId FROM songs WHERE albumId IS NOT NULL)")
