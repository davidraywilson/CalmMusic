...
object LocalMusicScanner {
    ...
    suspend fun scanFolders(
        ...
        val (unchanged, changed) = candidates.partition { candidate ->
            ...
            existing != null &&
                    existing.sourceType == SourceType.LOCAL_FILE &&
    ...
    fun buildSongEntityFromFile(
        ...
        // Industry standard: when an explicit Album Artist tag is set, use it to key
        // the album (e.g. "Various Artists" for compilations). When it's absent, key
        // the album by name only so that different track artists on the same album
        // are grouped together rather than creating duplicate albums.
        val hasExplicitAlbumArtist = explicitAlbumArtist != null
        ...
        val albumId = if (albumNameIdComponent != null) {
            if (hasExplicitAlbumArtist && albumArtistIdComponent != null) {
                // Explicit Album Artist: key by artist + album (standard single-artist album)
                "LOCAL_FILE:${albumArtistIdComponent}:${albumNameIdComponent}"
            } else {
                // No explicit Album Artist: key by album name only to avoid splitting
                "LOCAL_FILE::${albumNameIdComponent}"
            }
        } else existingAlbumId

        val albumArtistId = if (albumNameIdComponent != null && albumArtistIdComponent != null) {
            "LOCAL_FILE:$albumArtistIdComponent"
        } else null
